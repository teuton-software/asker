...
class ConceptAIScreenFormatter
  ...
  def export
    ...
    total_sa=total_sb=total_sc=total_sf=total_si=total_ss=0

    my_screen_table = Terminal::Table.new do |st|
      st << ['Concept','Questions','Entries','xFactor','a','b','c','f','i','s']
      st << :separator
    end
    ...
    @concepts_ai.each do |concept_ai|
      if concept_ai.process?
        ...
        my_screen_table.add_row [Rainbow(concept_ai.name).color(:green), t, e, factor, sa, sb, sc, sf, si, ss]

        total_q+=t; total_e+=e; total_c+=1
        total_sa+=sa; total_sb+=sb; total_sc+=sc; total_sf+=sf; total_si+=si; total_ss+=ss
      end
    end

    my_screen_table.add_separator
    my_screen_table.add_row [ Rainbow("TOTAL = #{total_c.to_s}").bright,Rainbow(total_q.to_s).bright,Rainbow(total_e.to_s).bright,Rainbow((total_q.to_f/total_e.to_f).round(2)).bright, total_sa, total_sb, total_sc, total_sf, total_si, total_ss ]
